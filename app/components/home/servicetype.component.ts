...
export class ServiceTypeComponent {
    ...
    ngOnInit() {
        ...
        if (this.webapi.netConnectivityCheck()) {
            ...
            self.webapi.getEMRComplete_http().subscribe(data => {
                xml2js.parseString(data._body, { explicitArray: false }, function (err, result) {
                    self.webapi.loader.hide();
                    if (result.APIResult_EMRComplete.Message == "Session expired, please login using MemberLogin screen to get a new key for further API calls") {
                        self.webapi.logout();
                    }
                },
                    error => {
